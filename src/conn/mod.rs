...
impl Conn {
    ...
    pub async fn get_binlog_stream(
        mut self,
        request: BinlogRequest,
    ) -> Result<
        Pin<Box<dyn futures_core::stream::Stream<Item = Result<mysql_common::binlog::Event>>>>,
    > {
        use futures_util::stream;
        use mysql_common::binlog::{BinlogVersion::Version4, EventStreamReader};

        // We'll disconnect this connection from a pool.
        self.inner.pool = None;
        self.request_binlog(request).await?;

        enum State {
            Iterating(Conn, EventStreamReader),
            Done,
        }

        let state = State::Iterating(self, EventStreamReader::new(Version4));

        let event_stream = stream::unfold(state, |state| async move {
            loop {
                match state {
                    State::Iterating(mut conn, mut esr) => match conn.read_packet().await {
                        Ok(packet) if packet.get(0) == Some(&0) => {
                            let event_data = &packet[1..];
                            match esr.read(event_data) {
                                Ok(event) => return Some((Ok(event), State::Iterating(conn, esr))),
                        ...
                        Ok(packet) if packet.get(0) == Some(&254) => {
                            ...
                            match eof_packet {
                                ...
                                Err(_) => {
                                    return Some((
                                        Err(DriverError::UnexpectedPacket { payload: packet }
                                            .into()),
                                        State::Done,
                                    ))
                                }
                        ...
                        Ok(packet) => {
                            return Some((
                                Err(DriverError::UnexpectedPacket { payload: packet }.into()),
                                State::Done,
                            ))
                        }
