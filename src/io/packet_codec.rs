    ...
    fn decode(&mut self, buf: &mut BytesMut) -> Result<Option<(RawPacket, u8)>, io::Error> {
        if self.chunk_len >= 0 {
            if self.chunk_len as usize <= buf.len() {
                ...
                println!("GOT CHUNK {} {:02x?}", chunk_len, &chunk[..::std::cmp::min(chunk_len, 32)]);
                ...
                if chunk_len == MAX_PAYLOAD_LEN {
                    ...
                    println!("GOT PACKET {} {:02x?}", packet_data.len(), &packet_data[..::std::cmp::min(packet_data.len(), 32)]);
            ...
            if buf.len() < 4 {
                ...
                println!("GOT CHUNK HEADER {} {}", self.chunk_len, self.seq_id);
                self.decode(buf)
            }
        }
    }
            ...
            None => {
                if buf.is_empty() {
                    ...
                    Err(io::Error::new(io::ErrorKind::Other,
                                       format!("bytes remaining on stream: {:02x?}", &buf[..])).into())
                }
            }
        }
    }
